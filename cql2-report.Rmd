---
title: "Report: rstac CQL2 extension development"
...
Earth observation images provide a reliable and consistent way to monitor Earth's surface. In the last decades, the number of orbital instruments for Earth observation has increased [@finer_combating_2018]. Cloud computing has been adopted by industry and academy to store, access, process, and distribute big Earth observation data [@wagemann_user_2021]. In this context, the Spatio-temporal Assets Catalog (STAC) [https://stacspec.org] protocol has become a standard to describe and search EO data cloud service providers. 

The STAC protocol complies with the OGC API Features standard (OAFeat) [@ogc_ogc_2019]. STAC enables users to search and access satellite images. The STAC specification allows the development of new extensions to support more specific and complex functionality, including the Common Query Language 2 (CQL2) extension [@ogc_common_2022]. CQL2 is part of OAFeat standard; it provides a complex way to search and retrieve spatial-temporal assets. CQL2 is a powerful query language that enables users to create complex filtering criteria in a expressive way. Using CQL2 in a STAC service, users can search for spatial assets using a variety of metadata information such as cloud cover and spatial relations. 

This report describes the development of an updated version of the rstac R package [@simoes_rstac_2021] that provides full support for the CQL2 specification as part of queries that use the STAC protocol. The rstac package can be accessed at https://github.com/brazil-data-cube/rstac and is also available on the Comprehensive Archive Network (CRAN).

# Project milestones

This section describes the implementation of the authors proposal that was approved by the Radiant Earth Foundation and STAC Project Steering Committee for the advance of STAC ecosystem programme.

## Milestone 1 - Proof of concept version with examples that validate the idea of using the package

Status: concluded in July 2022. 
...
Status: concluded in September 2022. 

The CQL2 standard includes a set of conformance tests [@ogc_ogc_2019]. We have validated our implementation using all conformance tests that provide a test dataset. The test dataset used in rstac conformance tests is provided by "Test Dataset for the CQL2 Test Suite" [https://cql2test.ldproxy.net/ne110m4cql2], derived from Natural Earth [https://www.naturalearthdata.com]. Besides this test suite, we also have implemented tests using Microsoft Planetary Computer STAC service [https://planetarycomputer.microsoft.com/api/stac/v1]. The list of conformance tests implemented in rstac is listed in appendix B.


## Milestone 3 - Pre-release version with documentation and testing

Status: concluded in November 2022. 
...
Status: concluded in January 2023.

We have submitted `rstac` 0.9.2 version to CRAN; R users can now install the package directly from CRAN. Package documentation has been updated with two new tutorials. The first tutorial shows how to create CQL2 expressions with `rstac`. There is also a tutorial on how to access Planetary Computer data using `rstac` and CQL2. We will submit this tutorial to Planetary Computer repository [https://github.com/microsoft/PlanetaryComputerExamples].


# Concluding remarks

This report describes the results of the CQL2 extension development in `rstac` package. The project enumerated four milestones that were concluded as described above. Current version of the `rstac` package can be accessed at [https://github.com/brazil-data-cube/rstac] under MIT license. The `rstac` package is open to the community and follow the ”Contributor Covenant - version 1.0.0” code of conduct. We updated package's documentation and have prepared new tutorials on how to use `rstac` and its new CQL2 functionality.
